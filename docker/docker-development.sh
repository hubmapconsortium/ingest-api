...
if [[ "$1" != "check" && "$1" != "config" && "$1" != "build" && "$1" != "start" && "$1" != "stop" && "$1" != "down" ]]; then
    ...
    if [ "$1" = "check" ]; then
        ...
        docker-compose -f docker-compose.yml -f docker-compose.dev.yml -p ingest-api build
    elif [ "$1" = "start" ]; then
        docker-compose -f docker-compose.yml -f docker-compose.dev.yml -p ingest-api up -d
    elif [ "$1" = "stop" ]; then
        docker-compose -f docker-compose.yml -f docker-compose.dev.yml -p ingest-api stop
    elif [ "$1" = "down" ]; then
        docker-compose -f docker-compose.yml -f docker-compose.dev.yml -p ingest-api down
    fi
fi
